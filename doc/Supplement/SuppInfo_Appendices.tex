...
%\url{https://github.com/colin-olito?tab=repositories}
...
%\subsection*{Lenormand \& Roze~(2022)}

%A recent simulation study of Y recombination arrest and sex-specific regulatory evolution inadvertently shed light on "sheltering hypotheses", and provides some indication of the robustness of the major assumptions in our Wright-Fisher simulations. \citet{LenormandRoze2022} used individual based simulations of SLR-expanding inversions on still-recombining sex chromosomes. Their focus was to study the consequences of sex-specific regulatory evolution subsequent to the fixation of SLR-expanding inversions, and they included additional assumptions (such as the possibility that fixed inversions were replaced by "reversions" to the original chromosomal arrangement). However, they assumed that these inversions initially fixed while infuenced by selection due solely to segregating partially recessive deleterious mutations (see \citealt{LenormandRoze2022} Supplementary Material). They explain in the main text of their article:

%\begin{quote}
%		To understand the dynamics of sex chromosome evolution in our model, first consider the case where the cis and trans regulators do not mutate. In this case, all inversions on the Y are eventually reversed and lost. This occurs in two steps: First, an inversion appears on a given Y and "freezes" a segment of the chromosome. If by chance this Y carries relatively few or milder deleterious mutations, this "lucky" inversion has a selective advantage. Consequently, it tends to fix among Y chromosomes, causing recombination suppression in this portion of the sex chromosomes. Larger inversions are overrepresented among these lucky inversions, as they contain more genes and exhibit a larger fitness variance (25) (fig. S1A). Once fixed, these Y chromosomes start accumulating deleterious mutations as a result of selective interference. Fitness declines faster for larger inversions because of stronger selective interference (fig. S1B).
%\end{quote}

%\noindent In their supplement (fig.~S1A,), they show both that larger indeed have larger fitness variance, but also degrade faster due to accumulation of deleterious mutations than smaller ones. Yet, they also found that smaller inversions had a higher probability of being fixed and subsequently stabilized (their fig.~S2C).
...
%	Y^I_{(t + 1)} = Y^I_t  \Bigg[ \prod_{i=1}^{r}\Big(1 - s \big(h p^{D}_{X_{f,i},t} + q^{D}_{X_{f,i},t} \big) \Big) \prod_{j=1}^{n-r} \Big(1 - s \big(h (p^{W}_{Y^I,t} q^{W}_{X_{f,j},t} + q^{W}_{Y^I,t} p^{W}_{X_{f,j},t}) + q^{W}_{Y^I,t} q^{W}_{X_{f,j},t} \big) \Big) \Bigg] \Bigg/ \overline{w}^Y
%\end{equation}

%\noindent where 
%\begin{align*}\label{eq:wBarY-multiLocProd}
%	\overline{w}^Y = Y^I_{(t + 1)} = Y^I_t & \Bigg[ \prod_{i=1}^{r}\Big(1 - s \big(h p^{D}_{X_{f,i},t} + q^{D}_{X_{f,i},t} \big) \Big) \prod_{j=1}^{n-r} \Big(1 - s \big(h (p^{W}_{Y^I,t} q^{W}_{X_{f,j},t} + q^{W}_{Y^I,t} p^{W}_{X_{f,j},t}) + q^{W}_{Y^I,t} q^{W}_{X_{f,j},t} \big) \Big) \Bigg] \times \\
%	&(1 - Y^I_t) \left[\begin{array}{c}
%											\prod_{i=1}^{r} \bigg( 1 - s \Big( h \big( p^{D}_{X_{f,i},t} q^{D}_{Y_{i},t} +  q^{D}_{X_{f,i},t} p^{D}_{Y_{i},t} \big) + q^{D}_{X_{f,i},t} q^{D}_{Y,t} \Big) \bigg)\\
%											\prod_{j=1}^{n-r} \bigg( 1 - s \Big( h \big( p^{W}_{X_{f,i},t} q^{W}_{Y_{i},t} +  q^{W}_{X_{f,i},t} p^{W}_{Y_{i},t} \big) + q^{W}_{X_{f,i},t} q^{W}_{Y_{i},t} \Big) \bigg)
%											\end{array} \right] \numberthis
%\end{align*}
...
\section{Supplementary Figures} \label{SuppFigs-App}
 ...
 \begin{figure}[htbp]
 \centering
 \includegraphics[scale=0.55]{../../figures/delMutAccumulationAutoVsSLR}
